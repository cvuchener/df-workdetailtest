...
QCoro::Task<> WorkDetail::assign(int unit_id, bool assign)
{
	...
	Q_ASSERT(thisptr);
	auto set_assignment = [this, unit_id](bool assign, ChangeStatus status) {
	...
	auto old_assignment = isAssigned(unit_id);
	...
	assignment->set_unit_id(unit_id);
	assignment->set_enable(assign);
	set_assignment(assign, WorkDetail::Pending);
	...
	if (!r) {
		...
		set_assignment(old_assignment, WorkDetail::Failed);
		co_return;
	}
	...
	const auto &assign_result = r->assignments(0);
	if (!assign_result.success()) {
		qCWarning(DFHackLog) << "editWorkDetail failed" << assign_result.error();
		set_assignment(old_assignment, WorkDetail::Failed);
		co_return;
	}
	set_assignment(assign, WorkDetail::NoChange);
}
